...
async def setup_alexa(hass, config_entry, login_obj: AlexaLogin):
    ...
    def _build_last_called_payload(
        ...
        if not isinstance(last, dict):
            _LOGGER.debug(
                "%s: last_called reject: not_dict last=%r",
                hide_email(email),
                last,
            )
            return None
        ...
        if not _valid_voice_summary(summary):
            _LOGGER.debug(
                "%s: last_called reject: invalid_summary summary=%r response=%r",
                hide_email(email),
                summary,
                last.get("response") if isinstance(last, dict) else None,
            )
            return None
        summary = summary.strip()
        ...
        if not serial_num:
            _LOGGER.debug(
                "%s: last_called reject: missing_serial summary=%r response=%r raw=%r",
                hide_email(email),
                summary,
                response,
                last,
        ...
        if ts <= 0:
            _LOGGER.debug(
                "%s: last_called reject: invalid_timestamp ts_raw=%r parsed=%r raw=%r",
                hide_email(email),
                ts_raw,
                ts,
                last,
        ...
        if ts <= int(account.get("last_called_customer_history_ts") or 0):
            _LOGGER.debug(
                "%s: last_called reject: watermark ts=%r watermark=%r summary=%r",
                hide_email(email),
                ts,
                int(account.get("last_called_customer_history_ts") or 0),
                summary,
            )
            return None
        ...
        if serial_num not in existing_serials_local:
            _LOGGER.debug(
                "%s: last_called reject: unknown_device serial=%r known=%s",
                hide_email(email),
                serial_num,
                sorted(existing_serials_local),
            )
            return None

        _LOGGER.debug(
            "%s: %s Returning serialNumber: %s; timestamp: %s; "
            "summary: %s; response: %s",
            hide_email(email),
            "[_build_last_called_payload] ",
            serial_num,
            ts,
            summary,
            response,
    ...
    def _init_last_called_probe_worker(account: dict) -> None:
                ...
                for attempt in range(LAST_CALLED_RETRY_LIMIT + 1):
                    ...
                    try:
                        async with account_live["last_called_api_lock"]:
                            ...
                            start_time = (
                                #                                max(0, earliest_ts - LAST_CALLED_LOOKBACK_MS)
                                max(0, earliest_ts - rvh_window_ms)
                                if earliest_ts
                                #                                else int((time.time() * 1000) - LAST_CALLED_LOOKBACK_MS)
                                else int((time.time() * 1000) - rvh_window_ms)
                            )
                            #                            end_time = int(time.time() * 1000) + LAST_CALLED_LOOKBACK_MS
                        ...
                        if isinstance(records, list):
                            _LOGGER.debug(
                                "%s: last_called probe raw records (%s): %s",
                                hide_email(email),
                                trigger_cmd,
                                [
                                    {
                            ...
                            _LOGGER.debug(
                                "%s: last_called probe returned unexpected type (%s): %r",
                                hide_email(email),
                                trigger_cmd,
                                records,
                            )
                    ...
                    _LOGGER.debug(
                        "%s: last_called queue match (%s): queued=%s matched=%s resolved=%s",
                        hide_email(email),
                        trigger_cmd,
                        [
                            {
                    ...
                    if not payload:
                        ...
                        # All retries exhausted with no match — drop the stale
                        # queue entries so they don't block the next refresh.
                        stale_keys = {
                            (item.get("serial"), item.get("customer_id"))
                            for item in queue_snapshot
                        }
                        _remove_last_called_activity_queue_entries(
                            account_live, stale_keys
                        )
                        _LOGGER.debug(
                            "%s: last_called probe gave up after %d attempts (%s); "
                            "cleared %d stale queue entries",
                            hide_email(email),
                            attempt + 1,
                            trigger_cmd,
                            len(stale_keys),
        ...
        def _trigger_last_called_probe(
            ...
            if trigger_ts_ms is not None:
                ...
                # Manual refresh triggers clear any push watermark and any
                # previously queued (unresolved) push entries so the worker
                # proceeds directly to get_last_device_serial instead of
                # getting permanently stuck behind an old unresolved burst.
                if trigger_command in (
                    ...
                    account_live["last_called_activity_queue"] = []
                account_live["last_called_probe_trigger_cmd"] = trigger_command
