...
class AlexaRuntimeData:
    ...
    async def cleanup(self) -> None:
        ...
        if (
            self.notifications_refresh_task
            and not self.notifications_refresh_task.done()
        ):
            self.notifications_refresh_task.cancel()
            try:
                await self.notifications_refresh_task
            except asyncio.CancelledError:  # nosec B110 - Expected during cleanup
                pass

        if self.last_called_probe_task and not self.last_called_probe_task.done():
            self.last_called_probe_task.cancel()
            try:
                await self.last_called_probe_task
